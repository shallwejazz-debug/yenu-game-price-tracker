...
(function () {
  ...
  function showAdmin() {
    ...
    touch()
    startIdleTimer()
    ensureAtLeastOneRow()   // 자동 가져오기 기본 행 1개 보장
    loadSettings()
    loadGames()
  }
  ...
  function addGroupRow(name, aliases) {
    ...
    row.innerHTML =
      ...
      '<input type="text" class="ig-alias" placeholder="예: 발더스게이트3, BG3, Baldur\\'s Gate 3" />' +
  ...
  // 화면의 행들 → groups 배열로 조립
  //   { name: "대표이름", aliases: ["별칭1","별칭2", ...] }
  //   - 별칭이 하나도 없으면 대표 이름 자체를 검색어로 사용
  //   - 대표 이름과 별칭이 모두 비면 그 행은 건너뜀
  function getGroups() {
  ...
  async function doImport(dryRun) {
    ...
    try {
      // 백엔드는 groups 우선, titles는 하위호환. 여기서는 groups로 전송.
